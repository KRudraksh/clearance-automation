...
async function processAttachment(buffer, filename, job) {
  try {
      ...
      // Generate blob name
      const blobName = blobStorage.generateBlobName(job._id, docType, filename);

      // Upload to Azure Blob Storage
      const blobUrl = await blobStorage.uploadFile(buffer, blobName);
      console.log('File uploaded to blob storage:', blobUrl);

      // Update job with blob URL and document info
      const updateData = {
          $push: {
              documentsReceived: {
                  type: docType,
...
app.get('/jobs/:jobId/document/:docType', async (req, res) => {
  ...
  try {
      ...
      const document = job.documentsReceived.find(doc => doc.type === req.params.docType);
      if (!document) {
          return res.status(404).json({ error: 'Document not found' });
      }

      // Get file from blob storage
      const blobName = blobStorage.generateBlobName(job._id, req.params.docType, document.fileName);
      const fileBuffer = await blobStorage.downloadFile(blobName);
...
const streamToBuffer = require('stream/promises').pipeline;

// Initialize BlobServiceClient
const blobServiceClient = BlobServiceClient.fromConnectionString(process.env.AZURE_STORAGE_CONNECTION_STRING);
const containerClient = blobServiceClient.getContainerClient('clearance-docs');
...
class BlobStorageService {
  ...
  generateBlobName(jobId, documentType, originalFileName) {
      const extension = path.extname(originalFileName);
      return `${jobId}/${documentType}${extension}`;
  }
}
